...
<Layout title="Menu" description="Menu provides a way to display list of choices on a temporary dropdown.">
...
It appears when the user interacts with a trigger, most commonly [Button](/components/button), although other elements can be used. Dropdown is placed next to the element that opened it. If it is in position to be cut off the screen's or browser's edge, the `Menu` can instead appear at the opposite `placement`.

It's option is defined with `Menu.Item` subcomponent. It can incude label text, description, left icon, right icon, active state, destructive variant, and small notification icon. Display item as `disabled`, when it can be used under certain conditions, instead of completely hiding it.

`Menu.Category` should be used to separate various `Menu.Items` into groups. It's divider can be hidden by providing `noDivider` prop.
...
<BestPractices>
  <BestPractices.Recommended>
    ...
    - use simple `Menu` instead of simple [Dialog](/components/dialog), because they are less distruptive to the user's context
