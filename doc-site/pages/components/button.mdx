...
**Primary** Button is ideally used only once on a page to display the most important action we want the user to take. Usually, the default size is sufficient, but to draw more attention `large` size can be used. Use `small` size sparingly—in tight spaces or more complex user interfaces.

**Secondary** Button can be used multiple times on a page, in a combination with primary or plain Buttons to create an action hierarchy.

**Plain** Button should be used for supportive, less important actions.

**Destructive** Button expresses an action a user can't take back.
...
<BestPractices>
  ...
  <BestPractices.Discouraged>
    ...
    - for an icon-only button, provide `aria-label` for both button and icon components
