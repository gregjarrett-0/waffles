...
When using the default `mode`, any values for `min` and `max` can be provided and a percentage is calculated internally. This includes values that would result in a percentage over `100%`. This percentage is also always **rounded down**.

If the context of a Progress cannot be determined visually (but can from accessibility content such as `aria-label`), then additional text should be placed alongside the Progress to solve this. A custom label can be provided (via the `customLabel` prop), to replace the default label. This should continue to communicate the `value` of the Progress to maintain accessibility, but can be otherwise customized.

For additional accessibility, an `aria-label` should always be provided with context of the progress bar, e.g. "Course progress".
...
This `mode` should be used when a process consists of multiple complex tasks, with distinct milestones. It's recommended **not** to use this mode with fewer than two (2) steps, or greater than seven (7)&mdash;consider restructuring the flow instead.
