...
<Layout title="Empty State" description="Used when an area of a page has no content to show the user.">

## Usage guidance

An `EmptyState` is useful in scenarios where there is a need to show container even when there is no content. An example of this would be in the Groups app, when no groups currently exist. They have the benefit of helping the application not feel lifeless or unnecessary, as well as promoting required action in some cases.
