...
<Layout title="Badge" description="Small descriptive tags for indicating the status of an element or action.">
...
The content of a Badge will always be show as uppercase and is available in three different sizes: `small`, `medium` and `large`. In most cases, the default value should be used.

The colors of a Badge should never be overwritten, as they are chosen for suitable accessibility and brand requirements.

Badges should **never** be interactive and instead a [Button](/components/button) or the future `Tag` component used if interaction is desired.

## Best practices

Assets should be used to provide visual context or expand upon a written point. They should **not** contain icons or anything besides text.
