...
import DarkVariants from '../../examples/badge/variants-dark';
...
`Badge` is available in a variety of colors, via the `variant` prop. The `white` and `greyLight` variants should only be used on dark backgrounds, such as `navy`. These colors of a `Badge` should never be overwritten, as they are chosen for suitable accessibility and brand requirements.
