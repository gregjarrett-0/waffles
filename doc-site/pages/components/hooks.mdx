...
<Layout title="Hooks" description="Collection of various utility hooks, which help with developing even advanced non-standard behaviors.">
...
Occasionally for better user experience there is a need to hint that container is scrollable by either showing shadows or griadient masks. Depending on `orientation` (`horizontal` and `vertical` are available) and based on scroll position, show hints at the appropriate positions.

Container should have `overflow: auto` or `overflow: scroll` specified via CSS.

Hook is used by [Table](/components/table), [Tabs](/components/tabs), and [Dialog.Body](/components/dialog) components.
