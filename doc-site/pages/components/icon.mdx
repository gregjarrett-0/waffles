...
An `Icon` can be used together with a [Button](/components/button), but it's important to note that an `Icon` is not a button outside of this and so direct interactions should be avoided.

When used within a [Button](/components/button), provide the `Icon` via the [Button's](/components/button) `icon`, `iconLeft` or `iconRight` properties. In a similar fashion to this, it can also enhance the [Link](/components/link) component.
